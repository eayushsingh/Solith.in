  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4 animate-fade-in">
      <div className="w-full max-w-sm bg-[var(--bg)] border border-[var(--line-subtle)] rounded-2xl p-6 relative flex flex-col max-h-[80vh]">
        ...
        <h3 className="text-lg font-semibold text-[var(--ink)] mb-4">{title} ({ids?.length || 0})</h3>

        <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar space-y-3">
          {loading ? (
            <div className="flex justify-center items-center py-8 text-[var(--ink-tertiary)]">
              <Loader2 className="w-6 h-6 animate-spin" />
            </div>
          ) : error ? (
            <div className="text-center text-red-500 py-4 text-sm">{error}</div>
          ) : profiles.length === 0 ? (
            <div className="text-center text-[var(--ink-secondary)] py-8 text-sm">
              No users found.
            </div>
          ) : (
              ...
              <div key={profile.id} className="flex items-center gap-3 p-3 rounded-xl hover:bg-[var(--bg-secondary)] border border-transparent hover:border-[var(--line-subtle)] transition-all">
                {profile.photoUrl ? (
                  ...
                  <div className="w-10 h-10 rounded-full bg-[var(--line)] flex items-center justify-center">
                    <User className="w-5 h-5 text-[var(--ink-tertiary)]" />
                  </div>
                )}
                <div className="flex-1 min-w-0">
                  <h4 className="text-sm font-medium text-[var(--ink)] truncate">{profile.name}</h4>
                  <p className="text-xs text-[var(--ink-secondary)] truncate">Level {Math.max(1, Math.floor(profile.xp / 100))}</p>
                </div>
              </div>
            ))
          )}
